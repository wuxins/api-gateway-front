...
function CRUD(options) {
  ...
  const data = {
    ...
    status: {
      ...
      get cru() {
        let res = null
        const statusList = [this.add, this.edit, this.read]
        if (statusList.every(item => item === CRUD.STATUS.NORMAL)) {
          return CRUD.STATUS.NORMAL
        }
        [CRUD.STATUS.PREPARED, CRUD.STATUS.PROCESSING].some(status => {
        ...
        throw new Error('wrong crud\'s cru status')
      },
      // 标题
      get title() {
        if (this.add > CRUD.STATUS.NORMAL) return `新增${crud.title}`
        if (this.edit > CRUD.STATUS.NORMAL) return `编辑${crud.title}`
        if (this.read > CRUD.STATUS.NORMAL) return `查看${crud.title}`
        return crud.title
      }
    },
    msg: {
  ...
  const methods = {
     ...
     */
    toRead(data) {
      crud.status.read = CRUD.STATUS.PROCESSING
      crud.resetForm({ ...data })
    },
    /**
     * 启动添加
     ...
     */
    cancelCRU() {
      crud.status.read = CRUD.STATUS.NORMAL
      crud.cancelCU()
    },
    /**
     * 提交新增/编辑
